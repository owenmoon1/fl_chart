...
* **FEATURE** Add `maskData` support to [CandlestickChart](https://github.com/imaNNeo/fl_chart/blob/main/repo_files/documentations/candlestick_chart.md). You can now draw masks when a point is selected, with customizable positioning (right, left, top, bottom) and styling. The mask automatically excludes the candlestick body to provide clean visual separation.
* **FEATURE** Add `maskData` support to [LineChart](https://github.com/imaNNeo/fl_chart/blob/main/repo_files/documentations/line_chart.md). You can now draw masks when a point is selected, with customizable positioning (right, left, top, bottom) and styling. Useful for highlighting selected data points with overlay effects.
* **FEATURE** Add `touchDelay` support to all chart types. You can now delay touch callback invocation by a specified duration, which is particularly useful when charts are inside scrollable widgets to avoid gesture conflicts. The delay is automatically cancelled if a pan gesture starts, allowing scroll views to handle the gesture properly.
